...
5. **JSON 協議** — CLI stdout 穩定 JSON，進度和診斷走 stderr
...
收到使用者意圖
  ↓
courseape agent doctor --json
  ↓
判斷 logged_in / profile_set / 資料齊全
  ├─ logged_in = false → 告知使用者執行 courseape login
  ├─ profile_set = false → courseape agent setup --json
  │   → Agent 從結果判斷是否需要確認系所
  ├─ 缺資料 → courseape agent prepare <task> --json
  └─ 資料齊全 → 進入對應 workflow
  ↓
執行 workflow
  ↓
...
```bash
courseape agent resolve "資管系" --json
```
...
執行：
```bash
courseape agent prepare graduation --json
```
...
執行：
```bash
courseape agent prepare planning --json
```
...
**流程**：
1. `courseape agent doctor --json` — 檢查現狀
2. 如果未登入 → 告知使用者執行 `courseape login`
3. `courseape agent setup --json` — 自動推導 profile
4. 確認系所（如果需要）
5. 告知使用者設定完成
...
**流程**：
1. `courseape agent doctor --json` — 檢查狀態
2. 自動補齊所有缺失資料（登入、profile、prepare graduation）
3. 讀取修業辦法 PDF（使用 PDF Skill）
4. 匯出已分析成績：
   ```bash
   courseape data export --scope grades --output json
   ```
5. 匯出歷史開課（含 OP_TYPE）：
   ```bash
   courseape data export --scope offerings --output json
   ```
6. 交叉比對，產出：
   - 已修課程摘要
   - 需重修課程（不及格/停修）
...
1. 匯出原始成績 HTML：
   ```bash
   courseape data export --scope grade-html --output json
   ```
2. 分析 HTML，對每一筆課程：
...
3. `courseape agent prepare planning --json` — 確認開課資料
4. 執行自動規劃：
   ```bash
   courseape courses plan --term <next_term> --json
   ```
5. 補充篩選（根據缺修課程搜尋開課）：
   ```bash
   courseape courses filter --term <next_term> --keyword <缺修課程名> --output json
   ```
6. 加入備選清單：
   ```bash
...
7. 檢查衝堂：
   ```bash
   courseape courses conflicts --term <next_term> --json
   ```
8. 顯示課表：
   ```bash
...
1. 取得課程資訊：
   ```bash
   courseape courses filter --term <term> --code <code> --output json
   ```
2. 搜尋網路：
   - 「<course_name> 中原 評價」
   - 「<teacher_name> 中原 心得」
...
1. 執行篩選：
   ```bash
   courseape courses filter --term <term> --keyword <query> --output json
   ```
